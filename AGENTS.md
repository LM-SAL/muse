...
- **Finalize every newly constructed dataset:** preserve its intrinsic attrs, explicitly copy or merge non-provenance source attrs with `update_attrs`, then call `add_history(..., sources=(...))` so every source lineage and the producing call are recorded. For concatenation, choose `combine_attrs` explicitly; never accept xarray's implicit first-input attrs.
...
- `remote_data`/`online` markers gate network tests (`remote_data_strict = true`); `chianti` gates tests requiring a local CHIANTI database; default runs exclude all of them. `mpl_image_compare` applied via `figure_test` decorator, not by hand.
...
- CHIANTI (`muse/instrument/linelist.py`): needs `[chianti]` extra + `XUVTOP` env var pointing at a local CHIANTI database. Live tests are `chianti`-marked and run in a dedicated CI job (`chianti` in `ci.yaml`) that downloads/caches the database; validation tests run offline without it.
- `.history/` = editor backup noise — ignore; excluded from pytest.
- Legacy `.isort.cfg` exists but ruff is source of truth for lint (isort rules `I` delegated to isort pre-commit hook).
