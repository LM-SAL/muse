...
On Linux without an NVIDIA GPU, install the slim CPU Torch build first (see :ref:`muse-installing-torch`) so the ``dev`` extra does not pull the multi-gigabyte CUDA stack; tox environments and CI already do this via ``UV_TORCH_BACKEND=cpu``.
