...
``muse`` is an instrument team-led, free and open-source Python toolkit for analyzing MUSE data.
...
But we do not assume you have any prior experience with MUSE data or the specific tools in ``muse``.

Since ``muse`` depends heavily on other packages in the scientific Python ecosystem, including ``sunpy``, ``ndcube``, ``astropy``, ``numpy``, and ``matplotlib``.
This tutorial explains the basics of the packages needed to use ``muse``, but is not a comprehensive tutorial for these packages and references their own introductory tutorials where relevant.
