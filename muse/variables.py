...
def _quantity_mapping(unit):
    ...
    def to_unit(mapping):
        return {key: u.Quantity(value).to(unit) for key, value in mapping.items()}
...
class InstrumentDefaults:
    ...
    All fields are validated and normalized on construction and on every later
    assignment.
    """
    ...
    mesh_transmission: dict | None = field(default=None, validator=_instance(dict))
    """
    ...
    lpi: dict | None = field(default=None, validator=_instance(dict))
    """
    ...
    main_lines_SG_wavelength: dict | None = field(default=None, validator=_instance(dict))
    """
    Center of the main spectral lines in angstrom, keyed by line name.
    """

    bands_SG: np.ndarray | None = field(default=None, converter=converters.optional(np.asanyarray))
    """
    ...
    exposure_times_SG: dict | None = field(default=None, converter=_quantity_mapping(u.s))
    """
    ...
    exposure_times_CI: dict | None = field(default=None, converter=_quantity_mapping(u.s))
    """
    ...
    target_logT: dict | None = field(default=None, validator=_instance(dict))
    """
    Target logT values for different solar conditions.
    """

    target_vdop: dict | None = field(default=None, validator=_instance(dict))
    """
    ...
    initial_wavelength_SG: xr.DataArray | None = field(default=None, validator=_instance(xr.DataArray))
    """
    Wavelength at SG_xpixel=0 for slit=0, in Angstroms.
    """

    channel_spectral_order: xr.DataArray | None = field(default=None, validator=_instance(xr.DataArray))
    """
    ...
    def instrumental_width_sg(self):
        ...
        future property.
        """
        return 0.0815 / FWHM_TO_SIGMA / self.channel_spectral_order
...
DEFAULTS_INSTRUMENT = InstrumentDefaults(**MUSE_DEFAULTS_DICT)
DEFAULTS_AIA = InstrumentDefaults(**AIA_DEFAULTS_DICT)
...
DEFAULTS_GFAT = GFATDefaults()
DEFAULTS_SDC = SDCDefaults()
DEFAULTS_SDC_BENCHMARK = SDCBenchmarkDefaults()
DEFAULTS_SVD = SVDDefaults()
