...
def test_match_fov_resampled_intensity_map(vdem_offgrid):
    """
    Before/after match_fov: off-grid input vs MUSE-grid output, (y, x) maps over
    logT+doppler_velocity.
    """
    out = match_fov(vdem_offgrid)
    fig, (ax_in, ax_out) = plt.subplots(1, 2, figsize=(11, 4), constrained_layout=True)
    vdem_offgrid.vdem.sum(dim=["logT", "doppler_velocity"]).plot(ax=ax_in)
    ax_in.set_title("input (off-grid)")
    out.vdem.sum(dim=["logT", "doppler_velocity"]).plot(ax=ax_out)
