...
from muse.variables import DEFAULTS_INSTRUMENT, FWHM_TO_SIGMA
...
@format_docstring("DEFAULTS_INSTRUMENT", transmission="mesh_transmission")
def gausslobes_peak(*, nspike=100, transmission=None):
    ...
    if transmission is None:
        transmission = DEFAULTS_INSTRUMENT.mesh_transmission
    if isinstance(transmission, dict):
        transmission = next(iter(transmission.values()))
...
@format_docstring("DEFAULTS_INSTRUMENT", lpi="lpi")
def gausslobes_distance(wave0, *, lpi=None, arcsec=False):
    ...
    """
    if lpi is None:
        lpi = DEFAULTS_INSTRUMENT.lpi
    if isinstance(lpi, dict):
        lpi = next(iter(lpi.values()))
    lpi = float(np.ravel(lpi)[0]) if np.ndim(lpi) else float(lpi)
    asec = np.pi / 180 / 3600
    spacing = wave0 * 1e-10 / (0.0254 / lpi) / asec
    if arcsec:
        return spacing
    xpix_scale = DEFAULTS_INSTRUMENT.dx_pixel_SG.to_value(u.arcsec)
    ypix_scale = DEFAULTS_INSTRUMENT.dy_pixel_SG.to_value(u.arcsec)
...
@format_docstring(
    "DEFAULTS_INSTRUMENT",
    wavelength=DEFAULTS_INSTRUMENT.main_lines_SG_wavelength["Fe XV 284.163"],
    fwhm_x="psf_fwhm_x",
    fwhm_y="psf_fwhm_y",
    lpi="lpi",
...
def gausslobes_single_wavelength(
    *,
    ...
    if wavelength is None:
        wavelength = DEFAULTS_INSTRUMENT.main_lines_SG_wavelength["Fe XV 284.163"] * u.AA
    if fwhm_x is None:
        fwhm_x = DEFAULTS_INSTRUMENT.psf_fwhm_x
    if fwhm_y is None:
        fwhm_y = DEFAULTS_INSTRUMENT.psf_fwhm_y
    if lpi is None:
        lpi = DEFAULTS_INSTRUMENT.lpi
    if isinstance(lpi, dict):
        lpi = next(iter(lpi.values()))
    elif isinstance(lpi, list | tuple):
        lpi = lpi[0]
    if nslits is None:
        nslits = DEFAULTS_INSTRUMENT.number_of_slits_SG
    if nsteps is None:
        nsteps = DEFAULTS_INSTRUMENT.steps_per_raster_SG
    if oversample_x is None:
        oversample_x = DEFAULTS_INSTRUMENT.oversample_x_SG
    if oversample_y is None:
        oversample_y = DEFAULTS_INSTRUMENT.oversample_y_SG
    if slit_dim is None:
        if isinstance(DEFAULTS_INSTRUMENT.pixels_SG, u.Quantity):
            slit_dim = DEFAULTS_INSTRUMENT.pixels_SG.to_value(u.pix)
        else:
            slit_dim = DEFAULTS_INSTRUMENT.pixels_SG
    if transmission is None:
        transmission = DEFAULTS_INSTRUMENT.mesh_transmission
    if isinstance(transmission, dict):
        transmission = next(iter(transmission.values()))
    elif isinstance(transmission, list | tuple):
        transmission = transmission[0]
    if xpix_scale is None:
        xpix_scale = DEFAULTS_INSTRUMENT.dx_pixel_SG
    if isinstance(xpix_scale, u.Quantity):
        xpix_scale = xpix_scale.to_value(u.arcsec)
    if ypix_scale is None:
        ypix_scale = DEFAULTS_INSTRUMENT.dy_pixel_SG
    ...
    midy = int(noy / 2)
    midx = int(nox / 2)
    sigma_x = fwhm_x / FWHM_TO_SIGMA
    sigma_y = fwhm_y / FWHM_TO_SIGMA
