...
def test_gausslobes_distance_pixels() -> None:
    spacing = gausslobes_distance(284.163, lpi=70, arcsec=True)
    shift_x, shift_y = gausslobes_distance(284.163, lpi=70)
    assert_allclose(shift_x, spacing / DEFAULTS_INSTRUMENT.dx_pixel_SG.to_value(u.arcsec))
    assert_allclose(shift_y, spacing / DEFAULTS_INSTRUMENT.dy_pixel_SG.to_value(u.arcsec))


def test_gausslobes_distance_lpi_types() -> None:
    base = gausslobes_distance(284.163, lpi=70, arcsec=True)
    assert_allclose(gausslobes_distance(284.163, lpi=np.int64(70), arcsec=True), base)
    assert_allclose(gausslobes_distance(284.163, lpi=[70], arcsec=True), base)
    assert_allclose(gausslobes_distance(284.163, lpi={284: 70}, arcsec=True), base)
...
def test_axis_cuts() -> None:
    ...
    transmission = next(iter(DEFAULTS_INSTRUMENT.mesh_transmission.values()))
...
def test_wavelength_quantity_scalar() -> None:
    from_float = gausslobes_single_wavelength(wavelength=171.073)
    from_quantity = gausslobes_single_wavelength(wavelength=171.073 * u.AA)
    assert_allclose(from_quantity.values, from_float.values)


def test_wrapper_multi_wavelength() -> None:
    wavelengths = [108.355, 171.073, 284.163]
    stack = gausslobes(wavelength=wavelengths)
    assert dict(stack.sizes) == {"wavelength": 3, "x": 771, "y": 2049}
    single = gausslobes_single_wavelength(wavelength=171.073)
...
def test_wrapper_custom_dim_name() -> None:
    ...
    wavelengths = xr.DataArray([171.073, 284.163], dims=["line"])
    stack = gausslobes(wavelength=wavelengths)
...
def test_instrumental_width_sg_uses_shared_constant() -> None:
    width = DEFAULTS_INSTRUMENT.instrumental_width_sg
    expected = 0.0815 / FWHM_TO_SIGMA / DEFAULTS_INSTRUMENT.channel_spectral_order
    assert_allclose(width.values, expected.values)
    assert width.sel(channel=284).item() == 0.0815 / FWHM_TO_SIGMA
