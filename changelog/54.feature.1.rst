...
keyword that opens response files dask-backed, so resampling and synthesis
stay lazy and peak memory stays bounded by the on-disk chunks.
...
dask-backed operands so the contracted dims are single-chunk, keeping the
task graph and peak memory bounded.
